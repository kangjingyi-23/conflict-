...
访问日期：2026-08-16  
当前阶段：研究、产品判断、文档输出。未修改代码，未制作页面或视觉稿。
...
1. MVP 应优先服务“吵架后独自复盘的一方”，同时预留“邀请对方补充版本”的双人协作路径。理由是：吵架后的即时使用场景更可能由一方发起；但单方叙述存在事实偏差，产品必须显式声明“AI 只能分析你提供的信息，不能验证完整事实”，并把双方版本比较作为升级路径。
...
5. 市场空缺在于：多数情侣 App 做日常连接和问答，AI 聊天做泛化建议，CBT 日记做个人情绪结构化，冲突工具要么过于治疗化，要么偏“识别操控/红旗”。本产品可差异化为“中文语境、冲突事件级记录、单方/双方版本对照、理论约束的 AI 复盘、长期重复冲突模式追踪”。
...
| 未决 | 单人复盘还是双方共同使用 | 建议 MVP 单人优先，双人补充为可选 |
...
2. “描述双方观点”应设计为可选双版本，而不是强制双人。强制双人会提高进入门槛；但单方版本必须有明显局限声明。
...
普通情侣日记强调纪念、甜蜜、日常互动。本产品强调冲突事件的结构化记录、证据标注、修复行动和长期重复模式。

### 11.2 与 AI 聊天的差异

普通 AI 聊天可给建议，但容易受用户叙述带偏，也不一定提醒“单方叙述局限”。本产品应通过固定字段、理论约束、证据引用和安全分流降低随意判断。
...
恋爱测试常给类型、分数、匹配度。本产品不主张给人格/依恋/有毒伴侣标签，而是聚焦具体事件、具体行为和下一步修复。
...
1. 冲突记录表单：日期、主题、起因、时间线、双方关键原话/行为、当前状态。
2. 单方输入局限声明：报告开头固定提示“基于你提供的信息，不能验证完整事实或替对方发声”。
3. 结构化 AI 复盘：事实/解释/情绪/需要/升级行为/责任地图/修复建议。
4. 高风险内容识别与分流：暴力、威胁、自伤、跟踪、控制、性强迫等触发安全模式。
5. 历史记录与删除：查看、编辑、导出、彻底删除自己的记录。
6. 去人格化语言规范：不输出“他就是自恋/有毒/回避型人格”等诊断标签。
7. 隐私说明：明确数据用途、是否用于模型训练、分享边界、删除方式。

### 13.2 可以做

1. 邀请对方填写版本：MVP 可做轻量链接/码，但不是强制。
2. 双方版本差异表：事实差异、感受差异、需求差异、解释差异。
3. 修复话术生成：基于 NVC 和有效道歉结构生成 2-3 个版本。
4. 复盘提醒：24 小时后/一周后检查是否修复。
5. 主题标签和基础趋势：钱、时间、回应速度、边界、家务、亲密、未来规划等。
...
把“AI 审判”作为轻娱乐入口，但把真实价值落在“可追溯证据 + 双方需求 + 互动循环 + 责任行为 + 修复行动”上。第一版聚焦单方复盘和可选双人补充，建立隐私、安全和伦理边界，再进入 PRD 与原型阶段。
